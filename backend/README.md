...
| 运行配置 | `resources/application.yml` | 数据源、Redis、JWT、验证码 |
...
2. **Interceptor** `AuthInterceptor`：白名单（健康检查 / 验证码 / 登录 / 注册）直接放行；其它接口若 ThreadLocal 没有用户，返回「未登录」。
3. **Controller**：`@Valid` / `@Validated` 校验入参，调用 Service。
4. **Service / Mapper**：业务逻辑 + MyBatis-Plus 访问 `sys_user`。
5. **异常**：业务抛 `BizException`，参数错误由校验异常，最终都进 `GlobalExceptionHandler`，统一成 `ApiResult`。
6. **AOP**：方法上有 `@OperLog` 时，`OperLogAspect` 打操作日志（谁、做什么、耗时）。
